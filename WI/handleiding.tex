...
Als we haken om grote symbolen in \textit{displaymath} zetten, zoals bijvoorbeeld om een breuk heen, is het belangrijk de haakjes te schalen naar het symbool.
Dit kan worden gedaan door de \verb.\left. en \verb.\right. commandos toe te voegen aan de haakjes:
...
Als je een linkerhaakje wilt laten matchen met een rechterhaakje op een andere regel kan je de onzichtbare \verb|\right.| gebruiken. Dit komt bijvoorbeeld voor bij lange definities van verzamelingen:
...
Naast de tekens uit het Griekse alfabet en de standaard functies hebben we ook nog enkele sommen, producten, integralen etc. Deze symbolen kunnen zich aanpassen naar de omgeving waarin zij zich bevinden. Zo worden zij groter weergeven in \textit{displaymath} dan in \textit{math}. De symbolen hieronder worden het meest gebruikt:
\begin{center}
...
verkregen door de code \verb|\[\iint_D \text{ met } \iint\limits_D\]|, hierbij is de tweede correct.
...
\section{Tabellen}
Er zijn verschillende mogelijkheden om een tabel aan te maken in een \LaTeX-bestand. De meest gebruikte zijn de tabel (\verb|\begin{tabular}[positie]{opmaak}|)\\
en de array (\verb|\begin{array}[positie]{opmaak}|). Een array kan alleen worden aangemaakt in 
...
Het is ook mogelijk om afbeeldingen in te voegen bij \LaTeX. Hiervoor heb je het moet je bovenaan je code \verb|\usepackage{graphicx}| zetten. Deze package zorgt ervoor dat je afbeeldingen kan invoegen. Vervolgens gebruik je de code \verb|\includegraphics{naam_afbeelding.jpg}| om de afbeelding ook daadwerkelijk in te voegen. Om dit te doen moet de afbeelding die je wilt invoegen ook in de map staan waarin je \LaTeX-code ook staat. Let op: \LaTeX\ kan alleen \verb|.pdf|-, \verb|.png|- of \verb|.jpg|-bestanden lezen.  \\
Om het formaat van de afbeelding aan te passen kun je de code\\ \verb|\includegraphics[scale=0.5]{naam_afbeelding.jpg}|. Hierbij geef je de schaal van de afbeelding, in plaats hiervan kun je ook \verb|[width=0.9\textwidth]| gebruiken. Zo wordt de afbeelding ook aangepast naar de gewenste grootte.
...
\verb+\label{HierEenNaam}+ gebruiken. Als je \verb|\label{...}| eenmaal op de juiste
