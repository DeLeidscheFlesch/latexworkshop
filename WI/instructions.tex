...
\end{enumerate}
An online TeX editor has existed for a few years: Sharelatex\footnote{\url{https://www.sharelatex.com}}, which is for making \LaTeX\ documents \emph{in the cloud}. 
It works like Google Docs: it is accessible from every computer and all files you make will be saved online. 
This way, you can never forget your homework.

\newpage
\section{New document}


A \LaTeX\ document consists of 2 parts. The first part is the preamble. The things written here will be valid throughout the whole document, such as the right packages and fonts. The second part starts at \verb.\begin{document}. and ends at 
\verb.\end{document}.. The contents of your document are inbetween these two commands.

You can put comments in your .tex file, without them being displayed in your PDF. 
You can accomplish this by using \verb.%. in front of your commentary.

\subsection{Preamble} 

\begin{enumerate}

	\item At the top, you put \verb.\documentclass{article}.. This command tells your compiler what kind of text you are writing, in this case an article. 
		It specifies for example the font size. 
		The article class is the best in almost every situation. 
	
	\item Directly below this you put the packages you want to use. 
		These make it possible to add extra functies to \LaTeX{} that are not standard. 
	Add a package by putting \verb|\usepackage{packagename}| at the top of your document.
	...
	\begin{enumerate}[1)] 
        ...
        \item \verb.\usepackage{amsmath, amssymb}. A package that displays formulas and other mathematical object in a better looking way.
	...
	Between these two commands you put the body of your document.
...
At the top of your document body put \verb.\maketitle.. 
This makes sure the information you put in the preamble as the title will be at the right place in the document. In this case a title, author and date.
...
\paragraph{Paragraph}
Finally there are paragraph headers, you make these by typing \verb.\paragraph{paragraph name}.. This header is a paragraph type.

\subsubsection{Paragraph}
You can make paragraphs without a title by leaving an empty line between the text.

\subsubsection{Enumerations and lists}
You can make a non-numbered enumaration using \verb.\begin{itemize}.. 
An item in your enumaration is then made using \verb.\item.. 
Behind this command you type a space and then the text you want for this bullet-point. 
When your enumaration is done you close it using \verb.\end{itemize}.. 
Any following item can be put on a new line by again starting it with \verb.\item..
\begin{itemize}
\item This is the first item of a non-numbered enumaration
\item This is the second
\item This is the third
\end{itemize}
A numbered enumaration, as seen earlier in this guide, is made using \verb.\begin{enumerate}.. You create numbered items on you list by again using \verb.\item.. Again you close of with \verb.\end{enumerate}.. This will give you:
...
If you want a different item text or lay-out, you type \verb|\begin{enumerate}[a.]|. This will yield:
...
If you want formula's and equations in your reports then you will have to work in ``Math Mode". 
There are multiple way to enter Math Mode, the 3 most used methods are \textit{math}, \textit{displaymath} and \textit{align}, in which you can type mathematical formula's. 
In \textit{math} the mathematical symbols will just appear inline so on the textline itself\footnote{for some reason this does not always work for titles. You will need the package \textit{fixltx2e} to make this work}, like so: $a^n+b^n=c^n$. 
This environment can be invoked in multiple ways. 
The most used way is putting \verb|$|-signs around the equations.
...
Lastly we have the \textit{align} method, with this environment mathematical symbols will be aligned and they will get a reference number behind the equation, like so:
...
To display an equation like this use \verb|\begin{align}| and \verb|\end{align}|. 
	If you want to use this method without the reference numbers, type \verb|\begin{align*}| and \verb|\end{align*}|. 
	To list multiple equations below eachother, type \verb.\\. behind the equations. 
	Use the \verb|&|-sign to align the formulas correctly, most often you will want to use the \verb|&|-sign before the \verb|=|-sign in the formula. 
	If you are feeling adventurous, try finding out what happens if you use multiple \verb|&|'s in the same line.


\subsection{Superscript and subscript}
When working in Math Mode, subscript (\verb|_|) and superscript (\verb|^|) are often used. 
This way we can for instance display $a_1,a_2,\dots, a_{n-1}$ with the following code: \verb|a_1, a_2, \dots, a_{n-1}| and the equation $a^b\cdot a^c=a^{b+c}$ can be constructed by typing \verb|a^b \cdot a^c = a^{b + c}|. Pay attention if you want to subscript or superscript more than a single symbol you have to put \{ and \} around it.

\subsection{Brackets}

When putting brackets around larger parts of equations in displaymath, such as fractions, the brackets need to be scaled. 
This can be done by putting \verb.\left. and \verb.\right. before the brackets that need to be scaled:
\[\left| -\frac{1}{2} \right| ^3 = \left( \frac{1}{8} \right)\]

Is created by the following code:\\
\verb.\[\left| -\frac{1}{2} \right| ^3 = \left( \frac{1}{8} \right)\].

If you want to match an opening bracket with a closing bracket on a different line, use the invisible bracket \verb|\right.| denoted with a period, e.g. the end of a long set definition on a new line:
\[\left.\text{s.t. } f^2(x) < \frac{\epsilon}{2} \right\} \]
Is created using the following code:\\
\verb&\[\left.\text{s.t. } f^2(x) < \frac{\epsilon}{2} \right\} \]&
(Note the escape of the curly bracket here).

\subsection{Roots and fractions}
When using math you will often encounting roots and fractions. For displaying the fraction $\frac{a}{b}$ type the code \verb|\frac{a}{b}|.
For displaying the square root $\sqrt{a}$ we use \verb|\sqrt{a}| and for  $\sqrt[n]{a}$ we use \verb|\sqrt[n]{a}|. 

\subsection{Special symbols}
There are a lot of symbols that are used in math. 
In this paragraph a number of them will be named, but there are more. 
To quickly find which code belongs to which symbol you can use the website or the app Detexify\footnote{\url{http://detexify.kirelabs.org/}}. 
Example of symbols are integrals, but also for instance $\alpha$ and $\beta$. Below are the most used symbols of the Greek alfabet:
...
\end{tabular}
\end{center}
In addition symbols for sums, products, integrals etc. can be easily put in math mode. These symbols adapt to the math mode they are in, they are larger in \textit{displaymath} and super- and subscripts align nicely. Below are the most often used examples:
\begin{center}
...
by using the next code: \verb|\sum_{n=1}^{\infty} \frac{1}{n}|. You can make double integrals look cleaner by using the command \verb|\limits|. This makes sure the limits will align properly. For example compare:
\[\iint_D \text{ with } \iint\limits_D\]
obtained by the code \verb|\[\iint_D \text{ with } \iint\limits_D\]|. The second one is the correct notation.\\
If you want to use $\sum$ or $\prod$ in the \textit{math} environment and still want the upper en lower limits directly above or below the quantifier, you can use the command \verb|\displaystyle|. 
So for example $\displaystyle\sum_{n=0}^{\infty} \frac{1}{n}$ is obtained by the code \verb|\displaystyle\sum_{n=1}^{\infty} \frac{1}{n}|.\\\\

Finally there are a few more symbols that are useful to know. 
\begin{center}
\begin{tabular}{| l | l | l | l | l | l | l | l |}
\hline
& code & & code & & code & & code\\
...
(\verb|\begin{array}[position of the table]{layout of the table}|). You can only create an array in a math evironment. \\
...
Command & Result \\
\hline
\verb|t| & The top of the table is aligned on the bottom of the line.\\
\verb|b| & The bottom of the table is aligned to the bottom of the line. \\
\verb|c| & The center of the table is aligned at the bottom of the line. \\
\hline
\end{tabular}
\end{center}
In the definition of the layout you also need to define how many columns are in the table. To define the layout of the columns the following specifiers exist:
\begin{center}
\begin{tabular}{| l | l |}
\hline
...
\verb|p{x}| & The text is justified to the width of \verb|x|. \\
\verb|*{x}{layout}| & Repeats the structure in \verb|layout| \verb|x| times. \\
$\mid$ & Adds vertical line. \\
$\mid\mid$ & Adds double vertical line. \\
\verb|@{text}| & Uses \verb|text| as a vertical line. \\
\hline
\end{tabular}
\end{center}
...
It is possible to include images to \LaTeX. 
You first need to place \verb|\usepackage{graphicx}| in your preamble. 
This package makes sure you can include images. 
Secondly you use the code \verb|\includegraphics{picture.jpg}| to actually include the image. 
The image needs to be in the folder of your \LaTeX-code to actually include it. 
Be aware: \LaTeX\ can only include \verb|.pdf|-, \verb|.png|- and \verb|.jpg|-images with the \verb|graphicx| package.\\
You can use \verb|\includegraphics[scale=0.5]{picture.jpg}| to alter the size of your picture. 
You can give the scale of the images, but you can also use \verb|[width=0.9\textwidth]|. 
This way the images will also be scaled to the desired size.
...
\end{verbatim}
\end{center}
Commands can be given between the square brackets of \verb|\begin{figure}[]| for the placement of the picture. Below you can find some commands that are useful.
\begin{center}
\begin{tabular}{| l | l |}
\hline
Command & Result \\
...
For displaying code you can use the package \textit{listings}. This gives an environment \emph{lstlisting}. You put yout code between
...
You can easily implement the file by using:
\begin{verbatim}
...
When numbering things you can use the command \verb+\label{HereAName}+. 

When you have put \verb|\label{HereAName}| on the right spot, then you can use the number with \verb+\ref{HereAName}+. 
You can also use \verb+\pageref{HereAName}+ to refer to the page where the label is placed.

If you want a footnote in your text use
\verb+\footnote{This is a footnote}+\footnote{This is a footnote}.

For extensive citations use
\begin{verbatim}
...
\end{thebibliography}
\end{verbatim}
Afterwards, you can use \verb.\cite{image}. to refer to the url with more information about the image.

\textbf{For references it is important to compile the code twice. Otherwise questionmarks or outdated numberings will appear}.

\section{Other interesting packages}
Beside all this, there are several other packages to make your life easier. You should search for them on the internet!
\begin{itemize}
	\item \verb&\usepackage{fancyhdr}& For intelligent headers en footers on your page
	\item \verb&\usepackage[section]{placeins}& Enforce that floats such as figures do not cross sections. In addition the command \verb&\FloatBarrier& can be used to manually define a barrier for all floating objects.
	\item \texttt{latexmk} Is able to automise the whole compilation process.
	...
	\item Lion has its own \LaTeX-class for writing templates\cite{thesistemplate}. This basic template does a lot of the work for you, for example the layout of the front page.
