...
Wil je formules in je verslag dan moet je gaan werken in een ``wiskundige omgeving". 
Er zijn meerdere manieren om deze aan te roepen, de drie vaakst gebruikte zijn \textit{math}, \textit{displaymath} en \textit{align}, waarin je wiskundige formules kunt typen. 
Bij \textit{math} komen de wiskundige symbolen in de regel\footnote{Om bepaalde redenen werkt dit soms niet in koppen. 
Je zult het package \textit{fixltx2e} moeten toevoegen om het te laten werken} te staan, zoals $a^n+b^n=c^n$. 
Deze omgeving kun je op verschillende manieren aanroepen. 
Zoals alle omgevingen in \LaTeX\ werkt \verb|\begin{math}...\end{math}|, maar dit is veel typwerk. De bekendste en veel kortere manier is het zetten van de wiskundige formules tussen \verb|$|-tekens, gebruik deze, aangezien dat veel typwerk scheelt.\\
De \textit{displaymath} omgeving wordt gebruikt op de wiskundige symbolen te centreren, zoals
\[ \frac{\hbar^2}{2m}\nabla^2\Psi + V(\mathbf{r})\Psi= -i\hbar \frac{\partial\Psi}{\partial t}.\]
Je kunt deze omgeving aanroepen door het tussen de volgende haken te plaatsen: \verb|\[|\dots\verb|\]|.\\
...
\int_{-\infty}^{\infty} \frac{\cos x}{x^2 + 1} &= \frac{\pi}{e}
\end{align}
Om een vergelijking zo weer te geven gebruik je \verb|\begin{align}| en \verb|\end{align}|. 
Wil je dit zonder verwijzingen gebruiken, dan type je \verb|\begin{align*}| en \verb|\end{align*}|. 
Om meerdere vergelijkingen onder elkaar te zetten, type je achter een vergelijking \verb.\\.. Gebruik het \verb|&|-teken om de formules goed onder elkaar te zetten, meestal gebruik je het voor het \verb|=|-teken. Als je zin hebt om te experimenteren, probeer dan uit te zoeken wat er gebeurt als je meerdere \verb|&|'s in dezelfde regel gebruikt.
